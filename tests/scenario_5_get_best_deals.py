...
@pytest.mark.asyncio
async def test_get_best_deals_with_applicable_json_output(fake_agent: FakeAgent):
    # job_goal = "Get summary for campaign BK Sept 20OFF"
    productId = os.environ.get("VF_PRODUCT_BURGER_ID_3")
    job_goal = f"""
    Find best deal for customer with email test1@voucherify.io which has in the bucket one quantity of product with id {productId} for 15.50$ and 3 quantities of product with metadata 'category' set to 'Meat' for 10$ each. Return result in json format consistent with example below.
    """ + """
    Example output:
    {
        "best_deals": [
